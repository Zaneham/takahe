         ...
         const mp_bind_t *tbl, const cd_lib_t *cd, FILE *fp)
{
    ...
    fprintf(fp, "// Target: %s\n\n",
            lib->name[0] ? lib->name : "unnamed library");
...
wires:
    /* ---- Wire declarations ---- */
    {
        ...
        for (i = 1; i < M->n_net; i++) {
            ...
            /* Deduplicate on the emitted name. Internal nets carry their
             * index, so matching base names alone dropped real wires and
             * the output referenced undeclared ones. */
            if (wdone) {
                char ob[64];
                for (k2 = i; k2 < M->n_net; k2++) {
                    if (strcmp(em_nnam(M, k2, ob, 64), wnm) == 0)
                        wdone[k2] = 1;
                }
            }
